...
运行 `npm run build:static` 输出到 `out/`。GitHub Pages 工作流会在 main 更新后构建和部署。页面路径为 `/lyric-grid/`，换仓库名称时需同步修改 `vite.static.config.ts` 中的 base。

静态版保留浏览器本地编辑和导入功能。AI 在浏览器直接请求用户配置的智谱官方接口，仍保留任务限制和提示词；如果官方接口不允许跨域，请求会失败，静态托管不提供代理。密钥由用户自行填写，不内置在发布文件中。换网站后，原域名浏览器存储不会自动迁移，请先在旧站导出工程，再在新站导入。
